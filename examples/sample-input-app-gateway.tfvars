...
app_gateway_config = {
  ...
  ## Backend trusted root certificate - Choose ONE option:
  backend_trusted_root_certificate = [{
    name = "<rootcert-name>"
    ## Option 1: Local file upload
    data = "<path-to-rootcert>"  ## Path to certificate file
    ## Option 2: Key Vault certificate name
    # certificate_name = "<cert-name-in-keyvault>"  ## Certificate name in Key Vault
  }]
  
  ## SSL certificate for HTTPS listener - Choose ONE option:
  ssl_certificate = [{
    name = "<listener-cert-name>"
    ## Option 1: Local file upload (PFX/P12)
    data = "<path-to-listener-cert>"  ## Path to PFX file
    password = "<password>"            ## PFX password
    ## Option 2: Key Vault certificate name
    # certificate_name = "<cert-name-in-keyvault>"  ## Certificate name in Key Vault
  }]
  
  identity_ids              = ["<identity-id-with-access-to-key-vault>"] ## Required if using certificate_name
...
## Key Vault configuration (required ONLY if using certificate_name option above)
# key_vault_name = "my-keyvault"
# key_vault_resource_group_name = "keyvault-rg"  ## Optional, defaults to main resource_group_name
