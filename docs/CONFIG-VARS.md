...
| aks_network_policy | Sets up network policy to be used with Azure CNI. Network policy allows to control the traffic flow between pods. | string | null | Possible values are `calico`, `azure` and `cilium`. Network policy `azure` (Azure Network Policy Manager) and `cilium` (Cilium Network Policy) are only supported for `aks_network_plugin = azure`. Network policy `calico` is supported for both `aks_network_plugin` values `azure` and `kubenet`. For more details see [network policies in Azure Kubernetes Service](https://learn.microsoft.com/en-us/azure/aks/use-network-policies).|
