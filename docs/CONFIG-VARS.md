...
NOTE: `subscription_id` and `tenant_id` are always required. `client_id` and `client_secret` are requried when using a Service Principal. `use_msi=true` is requered when using an Azure VM Managed Identitty.
