locals {
  ...
  backend_trusted_root_certs = !var.create_app_gateway ? [] : (var.app_gateway_config != null && lookup(var.app_gateway_config, "backend_trusted_root_certificate", null) != null ? [
    for i, cert in var.app_gateway_config.backend_trusted_root_certificate : {
      name                = coalesce(
        lookup(cert, "name", null),
        lookup(cert, "certificate_name", null),
        try(regex("^.*/secrets/([^/]+).*$", lookup(cert, "key_vault_secret_id", "")), null)
      )
      data                = lookup(cert, "data", null) != null ? filebase64(cert.data) : null
      key_vault_secret_id = coalesce(
        lookup(cert, "key_vault_secret_id", null),
        (lookup(cert, "certificate_name", null) != null && lookup(cert, "data", null) == null) ? try(data.azurerm_key_vault_certificate.backend_cert[i].secret_id, null) : null
      )
    }
  ] : [])
  ...
  ssl_certs = !var.create_app_gateway ? [] : (var.app_gateway_config != null && lookup(var.app_gateway_config, "ssl_certificate", null) != null ? [
    for i, cert in var.app_gateway_config.ssl_certificate : {
      ...
      key_vault_secret_id = coalesce(
        lookup(cert, "key_vault_secret_id", null),
        (lookup(cert, "certificate_name", null) != null && lookup(cert, "data", null) == null) ? try(data.azurerm_key_vault_certificate.ssl_cert[i].secret_id, null) : null
      )
    }
  ] : [])
  ...
  identity_list          = local.should_create_identity ? [azurerm_user_assigned_identity.appgw[0].id] : (var.identity_ids != null ? var.identity_ids : [])
