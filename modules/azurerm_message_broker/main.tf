...
resource "azurerm_servicebus_namespace" "mq" {
  name                = "${var.prefix}-mq"
...
resource "azurerm_servicebus_namespace_authorization_rule" "mq_config" {
  name         = var.message_broker_name
  namespace_id = azurerm_servicebus_namespace.mq.id
